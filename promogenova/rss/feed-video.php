...
for ($i=1;$i<count($video['idVideo']);$i++) {
        ...
        print "<item>";
           ...
           // Titolo
           $txtConv=$myobj->mb_convert_encoding($video['video'][$i]);
    	   ...
    	   // Descrizione
            ...
            $descr.="Video YOUTUBE pubblicato sul Canale di Promogenova";           
            if ($video['giorno'][$i]!="") {
            ...
            print "<enclosure url='".$video['url'][$i]."' length='0' type='video/wmv' />";
       	    ...
       	    // Link alla pagina
            $linkVideo=htmlentities($video['url'][$i]);
            print "<link>".$linkVideo."</link>";
