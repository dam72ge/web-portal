...
<p><b>Promogenova</b> &egrave; presente sui maggiori <i>social network</i>
<!-- addthis -->
<div class="addthis_horizontal_follow_toolbox"></div>
<!-- AddThis Follow BEGIN 
<div class="addthis_toolbox addthis_32x32_style addthis_vertical_style" style="position: relative; width:auto; height:500px; margin:0; padding:0; background-color: #F5F5F5">
<a class="addthis_button_facebook_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_twitter_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_google_follow" addthis:userid="103140042063670371403" rel="external"></a>
<a class="addthis_button_youtube_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_linkedin_follow" addthis:userid="promogenova-it" rel="external"></a>
<a class="addthis_button_pinterest_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_flickr_follow" addthis:userid="101423608@N05" rel="external"></a>
<a class="addthis_button_instagram_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_foursquare_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_vimeo_follow" addthis:userid="user30524635" rel="external"></a>
<a class="addthis_button_tumblr_follow" addthis:userid="promogenova" rel="external"></a>
<a class="addthis_button_rss_follow" addthis:userid="rss"></a>
</div>
<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=xa-50e5a31c3b9cf938"></script>
<!-- AddThis Follow END -->
 </p>

<!--h5 class="grigio">
<img src="lay/tick.png" alt="tick " /> Vetrine web<br />
<img src="lay/tick.png" alt="tick " /> Siti internet CMS<br />
<img src="lay/tick.png" alt="tick " /> Pubblicit&agrave; e supporto eventi<br />
<img src="lay/tick.png" alt="tick " /> Condivisione su social network<br />
<img src="lay/tick.png" alt="tick " /> Consulenze e idee per Internet<br />
<img src="lay/tick.png" alt="tick " /> Foto e Video per il web<br />
<img src="lay/tick.png" alt="tick " /> Assistenza a Negozi, Aziende e Professionisti<br />
</h5-->
...
<?php
  ...
  $oggi=date("Ymd");
    ...
    while($eventi=mysql_fetch_array($query)){
    if ($conta<3) { 
        ...
        $titolo=$myobj->convTxT($eventi['titolo']);
        ...
        $testo=$myobj->convTxT($eventi['testo']);
...
for ($i=1;$i<count($attivita['idAttivita']);$i++) {
	if ($homeVetr<5) {
        ...
        $txtConv=$mysql->convTxt($attivita['attivita'][$i]);
        print "<span class='testo nero'>".ucfirst($txtConv)."</span>";
        print "</a><br />";
        $txtConv=$mysql->convTxt($attivita['ragsoc'][$i]);
...
<?php
  $conta=0;
    ...
    if ($conta<4) {
        ...
        print "<br/>";
        $giorno=$myobj->convTxT($video['giorno']);
...
print "<br /><br />";
     ...
     while ($tit=mysql_fetch_array($query_p)){
     $conta++;
	 print "<a href='territorio/municipi.php?idM=".$tit['idM']."' rel='index'>".$tit['idM'].". ".$tit['municipio']."</a> ";
              ...
              while ($q=mysql_fetch_array($query_q)){
              print $q['quartiere']." ";
    }
    print "<br />";
    }
...
<?php
     ...
     while ($tit=mysql_fetch_array($query_c)){
     $nome=$myobj->convTxT($tit['comune']);
...
<?php
     ...
     $query_c=mysql_query($sql_c);
     while ($tit=mysql_fetch_array($query_c)){
     $nome=$myobj->convTxT($tit['provincia']);
...
<?php
     ...
     $query_c=mysql_query($sql_c);
     while ($tit=mysql_fetch_array($query_c)){
     $nome=$myobj->convTxT($tit['regione']);
...
<?php
  $conta=0;
    ...
    if ($conta<3) {
        ...
        $titolo=$myobj->convTxT($album['album']);    
        ...
        $giorno=$myobj->convTxT($album['giorno']);
        print "Data: <span class='verde'>".$giorno."</span> - ";
...
<?php
  $conta=0;
    ...
    $query=mysql_query($sql);			
    while($rete=mysql_fetch_array($query)){
    if ($conta<3 && $rete['rete']!="") {
        print "<div class='colonna-1-4'><p>";
        $nome=$myobj->convTxT($rete['rete']);
